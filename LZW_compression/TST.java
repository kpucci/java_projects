...
public class TST<Value> {
    ...
    public String longestPrefixOf(String s) {
        ...
        int length = 0;
        Node<Value> x = root; // Start at root
        int i = 0;
        while (x != null && i < s.length()) {   // while there's still a node and not end of string
            char c = s.charAt(i);           // Get character
            if      (c < x.c) x = x.left;   // If c is less than char at x, go to left
            else if (c > x.c) x = x.right;  // If c is greater than char at x, go to right
            else {                          // Otherwise
                i++;                            // Increment i (go to next character)
                if (x.val != null) length = i;  // Length of longest prefix is equal to current index
                x = x.mid;                      // Go down center
            }
        }
        return s.substring(0, length);      // Return a substring of length equal to longest prefix
    }
